...
import com.gudu.ads.module.helper.blurEffect.BlurImage
...
class NativeAdModelHelper(private val mContext: Activity) {
    ...
    private val TAG = "GuduAdmob_${javaClass.simpleName}"
    ...
    fun loadNativeAdvancedAd(
        ...
        if (isNeedToShowAd && GuduAdsConfig.with(fLayout.context).remoteConfigNativeAdvancedAds && fLayout.context.isOnline) {
            mFLayout = fLayout
...
//    }
    ...
    fun manageShimmerLayoutVisibility(
        ...
        mShimmerLayout = shimmerLayout
        if (isNeedToShowAd && GuduAdsConfig.with(fLayout.context).remoteConfigNativeAdvancedAds) {
            if (!isAdLoaded) {
                fLayout.removeAllViews()
